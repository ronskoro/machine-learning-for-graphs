...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        ### YOUR CODE HERE ### 
        alpha = np.zeros((T, K))
        # initialize the alpha array with the values for timestamp 1
        alpha[0] = np.multiply(pi[:,0], B[:,x[0]])

        for i in range(1, T):
            # A transpose * alpha_i-1
            prod = np.dot(A.T, alpha[i-1]) 
            alpha[i] = np.multiply(B[:,x[i]], prod)
    ...
    def backwards(self, x):
        ...
        for i in range(T-2, -1, -1):
            prod = np.multiply(B[:, x[i]], beta[i+1])
            beta[i] = np.dot(A, prod)
    ...
    def generate_sentence(self, sentence_length):
        ...
        K = A.shape[0]
        V = B.shape[1]
        sequence = np.zeros(sentence_length)
        
        print(pi[:,0])
        # generate Z_1
        z = np.random.choice(K, p=pi[:,0])
        #generate X_1
        sequence = [np.random.choice(V, p=B[z,:])]

        for i in range(1, sentence_length):
            z = np.random.choice(K, p=A[z,:])
            x = np.random.choice(V, p=B[z,:])
            sequence.append(x)
        
        sentence = self.X_to_sentence(sequence)
        return sentence        
